...
public class PagerSlidingTabStrip extends HorizontalScrollView {
    ...
    private boolean isIndicatorExpand = false;
    ...
    private int tabBackgroundResId = R.drawable.tt_slide_tab_bg;
    ...
    public PagerSlidingTabStrip(Context context, AttributeSet attrs, int defStyle) {
        ...
        isIndicatorExpand = a.getBoolean(R.styleable.PagerSlidingTabStrip_pstsIsIndicatorExpand, isIndicatorExpand);
    ...
    private void updateTabStyles(int textColor, int position) {
        for (int i = 0; i < tabCount; i++) {
    ...
    private void updateTabStyles() {
        ...
        for (int i = 0; i < tabCount; i++) {
            ...
            if (v instanceof TextView) {
                ...
                tab.setTextSize(TypedValue.COMPLEX_UNIT_PX, tabTextSize);
                tab.setTypeface(tabTypeface, tabTypefaceStyle);
                tab.setTextColor(tabSwitch && i != currentPosition ? tabDeactivateTextColor : tabTextColor);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (currentTab instanceof TextView && !isIndicatorExpand) {
            TextView text = (TextView) currentTab;
            float width = currentTab.getWidth();
            float margin = (width - getTextPaint().measureText(text.getText().toString())) / 2;
            lineLeft += margin;
            lineRight -= margin;
        }
        ...
        if (currentPositionOffset > 0f && currentPosition < tabCount - 1) {
            ...
            if (nextTab instanceof TextView && !isIndicatorExpand) {
                TextView text = (TextView) nextTab;
                float width = currentTab.getWidth();
                float margin = (width - getTextPaint().measureText(text.getText().toString())) / 2;
                nextTabLeft += margin;
                nextTabRight -= margin;
    ...
    public void updateIndicatorText(int position, String text) {
    ...
    public Paint getTextPaint() {
        ...
        Style style = mTextPaint.getStyle();
        mTextPaint.setStyle(style);
        mTextPaint.setTextSize(tabTextSize); // 指定字体大小
        mTextPaint.setColor(tabTextColor);
        mTextPaint.setFakeBoldText(false); // 粗体
        mTextPaint.setAntiAlias(true); // 非锯齿效果
        return mTextPaint;
    }
    
    public void setTextColorByTab(int textColor, int position) {
        updateTabStyles(textColor, position);
    }
}