...
import com.moa.rxdemo.utils.ColorUtils;


/**
 * Copyright (c) 2016 浙江海宁山顶动力科技有限公司 All Rights Reserved.
 * <p>
 * http://www.top4s.net
 * <p>
 * 自定义switchbutton
...
public class SwitchButton extends CompoundButton {
    ...
    private void init(AttributeSet attrs) {
        ...
        if (ta != null) {
            thumbDrawable = ta.getDrawable(R.styleable.SwitchButton_moaThumbDrawable);
            thumbColor = ta.getColorStateList(R.styleable.SwitchButton_moaThumbColor);
            margin = ta.getDimension(R.styleable.SwitchButton_moaThumbMargin, margin);
            marginLeft = ta.getDimension(R.styleable.SwitchButton_moaThumbMarginLeft, margin);
            marginRight = ta.getDimension(R.styleable.SwitchButton_moaThumbMarginRight, margin);
            marginTop = ta.getDimension(R.styleable.SwitchButton_moaThumbMarginTop, margin);
            marginBottom = ta.getDimension(R.styleable.SwitchButton_moaThumbMarginBottom, margin);
            thumbWidth = ta.getDimension(R.styleable.SwitchButton_moaThumbWidth, thumbWidth);
            thumbHeight = ta.getDimension(R.styleable.SwitchButton_moaThumbHeight, thumbHeight);
            thumbRadius = ta.getDimension(R.styleable.SwitchButton_moaThumbRadius, Math.min(thumbWidth, thumbHeight) / 2.f);
            backRadius = ta.getDimension(R.styleable.SwitchButton_moaBackRadius, thumbRadius + density * 2f);
            backDrawable = ta.getDrawable(R.styleable.SwitchButton_moaBackDrawable);
            backColor = ta.getColorStateList(R.styleable.SwitchButton_moaBackColor);
            backMeasureRatio = ta.getFloat(R.styleable.SwitchButton_moaBackMeasureRatio, backMeasureRatio);
            animationDuration = ta.getInteger(R.styleable.SwitchButton_moaAnimationDuration, animationDuration);
            fadeBack = ta.getBoolean(R.styleable.SwitchButton_moaFadeBack, true);
            tintColor = ta.getColor(R.styleable.SwitchButton_moaTintColor, tintColor);
            textOn = ta.getString(R.styleable.SwitchButton_moaTextOn);
            textOff = ta.getString(R.styleable.SwitchButton_moaTextOff);
            textMarginH = Math.max(textMarginH, backRadius / 2);
            textMarginH = ta.getDimension(R.styleable.SwitchButton_moaTextMarginH, textMarginH);
            autoAdjustTextPosition = ta.getBoolean(R.styleable.SwitchButton_moaAutoAdjustTextPosition, autoAdjustTextPosition);
            ta.recycle();
        }
        ...
        if (!mIsThumbUseDrawable && mThumbColor == null) {
            mThumbColor = com.moa.rxdemo.utils.ColorUtils.generateThumbColorWithTintColor(mTintColor);
    ...
    private Layout makeLayout(CharSequence text) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
             layout = StaticLayout.Builder.obtain(text,0, text.length(),mTextPaint, (int) Math.ceil(Layout.getDesiredWidth(text, mTextPaint))).build();
        }else{
            layout = new StaticLayout(text, mTextPaint, (int) Math.ceil(Layout.getDesiredWidth(text, mTextPaint)),
