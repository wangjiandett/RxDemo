...
public class SwipeToLoadLayout extends ViewGroup {
    ...
    protected void onFinishInflate() {
        ...
        if (childNum == 0) {
            ...
            mHeaderView = findViewById(R.id.tt_swipe_refresh_header);
            mTargetView = findViewById(R.id.tt_swipe_target);
            mFooterView = findViewById(R.id.tt_swipe_load_more_footer);
        } else {
        ...
        if  (mTargetView instanceof RelativeLayout) {
            RelativeLayout relativeLayout = (RelativeLayout) mTargetView;
            for (int i = 0; i < relativeLayout.getChildCount(); ++i) {
                if (relativeLayout.getChildAt(i) instanceof AbsListView) {
                    mTargetListView = relativeLayout.getChildAt(i);
                    break;
                } else {
                    mTargetListView = relativeLayout.getChildAt(0);
                }
            }
        }  else {
            mTargetListView = mTargetView;
        }
    ...
    protected boolean canChildScrollUp() {
        if (android.os.Build.VERSION.SDK_INT < 14) {
            if (mTargetListView instanceof AbsListView) {
                final AbsListView absListView = (AbsListView) mTargetListView;
